    ...
    try {
        ...
        categories.forEach((categoryName, categoryIndex) => {
            ...
            categorySection.innerHTML = `
                <h3 id="${categoryName.toLowerCase().replace(/\s+/g, '-')}" class="category-title">${categoryName}</h3>
                <div class="feature-buttons category-buttons" id="buttons-${categoryName}"></div>
                <div class="reasoning-process">
                    <div id="reasoning-container-${categoryName.replace(/\s+/g, '-')}" class="reasoning-container">
...
async function loadTrajectoryCase(fileDir, category) {
    const container = document.getElementById(`reasoning-container-${category.replace(/\s+/g, '-')}`);
    container.innerHTML = `
        <div class="reasoning-loading">
            <div class="loading-spinner"></div>
